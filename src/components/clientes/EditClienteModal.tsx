import { useState } from 'react';
import { ESTADOS_CLIENTES, type ClienteForm } from '../../data/clientesData';
import { updateCliente } from '../../services/clienteService';
import type { BaseCliente } from '../../utils/vendasDomain';
import { clienteToForm } from '../../services/clienteService';
import ModalShell from '../colaboradores/ModalShell';

type EditClienteModalProps = {
  cliente: BaseCliente;
  onClose: () => void;
  onSuccess: () => void;
};

function maskCdc(value: string) {
  return value.replace(/\D/g, '').slice(0, 4);
}

export default function EditClienteModal({ cliente, onClose, onSuccess }: EditClienteModalProps) {
  const [form, setForm] = useState<ClienteForm>(clienteToForm(cliente));
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const updateField = (field: keyof ClienteForm, value: string) => {
    setForm((prev) => ({ ...prev, [field]: value }));
  };

  const handleSubmit = async () => {
    if (!form.cdc.trim() || !form.nomeFantasia.trim() || !form.razaoSocial.trim()) {
      setError('Preencha CDC, Nome Fantasia e Razão Social.');
      return;
    }

    setSaving(true);
    setError(null);

    try {
      await updateCliente(cliente.id, form);
      onSuccess();
      onClose();
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Erro ao atualizar cliente.');
    } finally {
      setSaving(false);
    }
  };

  return (
    <ModalShell onClose={onClose}>
      <div className="colab-modal-header">
        <h2>Editar Cliente</h2>
        <p>Atualize os dados do cliente</p>
      </div>

      <div className="colab-form">
        <div className="colab-row">
          <label className="colab-field">
            <span>CDC</span>
            <input value={form.cdc} onChange={(e) => updateField('cdc', maskCdc(e.target.value))} />
          </label>
          <label className="colab-field">
            <span>CNPJ</span>
            <input value={form.cnpj} onChange={(e) => updateField('cnpj', e.target.value)} />
          </label>
        </div>
        <label className="colab-field full">
          <span>Razão Social</span>
          <input value={form.razaoSocial} onChange={(e) => updateField('razaoSocial', e.target.value)} />
        </label>
        <label className="colab-field full">
          <span>Nome Fantasia</span>
          <input value={form.nomeFantasia} onChange={(e) => updateField('nomeFantasia', e.target.value)} />
        </label>
        <div className="colab-row">
          <label className="colab-field">
            <span>Cidade</span>
            <input value={form.cidade} onChange={(e) => updateField('cidade', e.target.value)} />
          </label>
          <label className="colab-field">
            <span>Estado</span>
            <div className="colab-select-wrap">
              <select value={form.estado} onChange={(e) => updateField('estado', e.target.value)}>
                {ESTADOS_CLIENTES.map((uf) => (
                  <option key={uf} value={uf}>
                    {uf}
                  </option>
                ))}
              </select>
            </div>
          </label>
        </div>
        {error && <p className="colab-error">{error}</p>}
        <div className="colab-actions">
          <button type="button" className="colab-btn primary" disabled={saving} onClick={handleSubmit}>
            {saving ? 'Salvando...' : 'Salvar'}
          </button>
          <button type="button" className="colab-btn outline" onClick={onClose}>
            Cancelar
          </button>
        </div>
      </div>
    </ModalShell>
  );
}
